...
#     p_buy = np.asarray(day.prices_buy_forecast, dtype=float)
#     # load = np.asarray(day.demand_MW, dtype=float)
#     if day.allow_export:
#         p_sell = np.asarray(
#             day.prices_sell_forecast if day.prices_sell_forecast is not None else day.prices_buy_forecast,
#             dtype=float,
#         )
#     else:
#         p_sell = None
...
#         export_MW=(
#             exp.value.tolist()
#             if (day.allow_export and exp is not None and exp.value is not None)
#             else None
#         ),
...
#         confidence=(
#             [
#                 max(y_c.value[t], y_d.value[t], 1 - y_c.value[t] - y_d.value[t])
#                 for t in range(T)
#             ]
#             if y_c.value is not None and y_d.value is not None
#             else None
#         ),
#     )
...
def solve_daily_milp(
    ...
    p_buy = np.asarray(day.prices_buy_forecast, dtype=float)
    load  = np.asarray(day.demand_MW_forecast, dtype=float)
    if day.allow_export:
        p_sell = np.asarray(day.prices_sell_forecast if day.prices_sell_forecast is not None else day.prices_buy_forecast, dtype=float)
    else:
        p_sell = None
