...
# else:
# OUTPUT_JSONL = Path(f"train_qwen_soc_{REGION}_varyingduration_tab4.jsonl")
OUTPUT_JSONL = Path(f"test_qwen_soc_{REGION}_dur{int(duration_hours)}_tab4.jsonl")
...
async def main():
    _forecast_obj = EnergyDataLoader(region=REGION, data_version=data_version, forecast_type=forecast_type)
    _forecast_data = _forecast_obj.load_region_data()
    _forecast = await _forecast_obj.get_filtered_data(start_date= start_date, end_date= end_date)

    _actual_obj = EnergyDataLoader(region=REGION)
    _actual_data = _actual_obj.load_region_data()
    _actual = await _actual_obj.get_filtered_data(start_date= start_date, end_date= end_date)

    stats = await EnergyDataLoader.get_summary_stats_from_ag(_actual_data)
    ...
    # capacity_MWh = np.random.choice([24.72, 49.44, 74.16, 98.88])
        
    batterydetails = BatteryParams(
        capacity_MWh=capacity_MWh,   # 4-hour energy at 100 MW power: good for diurnal spreads
        ...
        cmax_MW=cmax_MW,        # 4-hour duration (capacity/4)
        dmax_MW=dmax_MW,        # symmetric charge/discharge
    ...
    with OUTPUT_JSONL.open("w", encoding="utf-8") as f_out:
        for i in range(len(_forecast_data)//DATA_POINTS):
            print(f"Solving day {i+1} of {len(_forecast_data)//DATA_POINTS}")

            forecast_prices, forecast_demand = records_to_arrays(_forecast_data[i*DATA_POINTS:(i+1)*DATA_POINTS])
            actual_prices, actual_demand = records_to_arrays(_actual_data[i*DATA_POINTS:(i+1)*DATA_POINTS])


            records = SolveFromRecordsRequest(
                battery=batterydetails,
                records=_forecast_data[i*DATA_POINTS:(i+1)*DATA_POINTS],
                dt_hours=DT_HOURS,
            ...
            day = DayInputs(
                ...
                prices_buy_forecast=actual_prices,
                demand_MW_forecast=actual_demand,
                prices_sell_forecast= actual_prices,
            )
            ...
            user_message = build_user_message(
                battery=batterydetails,
                df_day=_forecast_data[i*DATA_POINTS:(i+1)*DATA_POINTS],
                dt_hours=DT_HOURS,
                allow_export=ALLOW_EXPORT,
            )
            
            # batterydetails.capacity_MWh = np.random.choice([24.72, 49.44, 74.16, 98.88])
            # batterydetails.soc_init= np.random.choice([0.20, 0.40, 0.60, 0.80])
            # efficiency = np.random.choice([0.80, 0.85, 0.90, 0.95])
            # batterydetails.eta_d = efficiency
            # batterydetails.eta_c = efficiency

            assistant_message = build_assistant_message(sol, include_cost=True)

            date_str = _forecast_data[i*DATA_POINTS].timestamps.split(" ")[0]
            example = {
                "date": date_str,
